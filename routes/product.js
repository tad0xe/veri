...
router.get("/products/:title", async (req, res) => {
  try {
    ...
    // Find the product by title (case-insensitive match)
...
console.log(product)
    ...
    res.json({ success: true, product, overallRating });
  } catch (error) {
...
router.get("/categories/:categoryType", async (req, res) => {
  try {
    ...
    const category = await Category.findOne({ type: req.params.categoryType });
